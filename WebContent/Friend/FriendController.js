myapp.controller("FriendController", function($scope, $http, $location,$rootScope) 
	{
		...
		$http.get("http://localhost:8181/SocialCollaboration/getAllFriendRequest")
		...
		$scope.approve=function(friendId)
		{
			...
			$http.get("http://localhost:8181/SocialCollaboration/approvalFriendRequest/"+friendId)
...
*/			});
		...
		$scope.reject=function(friendId)
		{
			...
			$http.get("http://localhost:8181/SocialCollaboration/rejectFriendRequest/"+friendId)
			.success(function(response){
		...
		$scope.addFriend=function()
		{
			...
			$http.post("http://localhost:8181/SocialCollaboration/createFriendRequest",$scope.friend)
			.then(function(response){
